...
full guide lives at /docs on the dragon stt site.`,L=`dragon-stt.onnx goes here: int8, input inputs [1,T,80] float32,
...
the trained weights.`;function w(){const t=Object.entries(F).map(([o,n])=>["src/"+o.replace(/^(?:\.\.\/)+/,""),n]).sort((o,n)=>o[0].localeCompare(n[0]));return t.push(["public/models/vocab.json",JSON.stringify([...T,""],null,2)]),t.push(["public/models/readme.txt",L]),t.push(["readme.txt",R]),t}function V(){E(B(w()),"dragon-stt-starter.zip"),u.success("starter.zip downloaded")}function O(){navigator.clipboard.writeText(D(w())).then(()=>u.success("all sources copied"),()=>u.error("copy failed"))}const $=[["layout","project layout"],["install","install"],["audio","audio in"],["transcribe","transcribe"],["features","features only"],["reference","reference"],["keys","api keys"],["offline","offline checklist"]],q=`your-app/
  src/stt/
    audio.ts
    decode.ts
    engine.ts
  ...
  public/
    ...
    models/
      dragon-stt.onnx
      vocab.json`,H="bun add onnxruntime-web",_=`import * as ort from "onnxruntime-web"

ort.env.wasm.wasmPaths = "/ort/"
ort.env.wasm.numThreads = 1`,G=`import { blobToPcm } from "./stt/audio"
